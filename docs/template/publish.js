...
function buildNav(members) {
    ...
    var addFolder = function(array){
        array.forEach(info => {
            info.folder = path.basename(info.meta.path);
        });
        array.sort((a, b) => {
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    function logClassInfo(classInfo){
        console.log('------------ ' + classInfo.longname + ' ------------');
    ...
    if(members.classes){
        var classDict = {};
        members.classes.forEach(function(classInfo){
            var filename = classInfo.meta.filename;
            var info = classDict[filename] = classDict[filename]||{};
            if(classInfo.alias){
                info.alias = classInfo;
            }
            else{
                info.class = classInfo;
            }
            if(classInfo.longname === 'Cache'){
                // logClassInfo(classInfo);
            }
        });

        members.classes = [];
        for(var filename in classDict){
            var info = classDict[filename];
            var classInfo = info.class;
            var aliasInfo = info.alias;
            if(classInfo){
                if(aliasInfo){
                    classInfo.params = aliasInfo.params;
                    classInfo.signature = aliasInfo.signature;
                }
                members.classes.push(classInfo);
            }
        }
    }
    // once for all
